...
// contract between the two methods (pczt 0.7 exposes no getter for the PCZT's
// native ZIP 32 / BIP 32 derivation metadata, so we stash our own copy).
// Defining them in one place keeps the writer and reader from drifting; the
